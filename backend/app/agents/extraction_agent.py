"""Extraction agent: turns raw invoice text into structured fields via the LLM."""
...
def _truncate_preserving_totals(text: str) -> str:
    """Cap invoice text length without silently dropping the totals/summary section"""
    if len(text) <= MAX_INVOICE_TEXT_CHARS:
        return text
    head = text[:HEAD_CHARS]
    tail = text[-TAIL_CHARS:]
    return f'{head}\n\n[... {len(text) - HEAD_CHARS - TAIL_CHARS} characters omitted ...]\n\n{tail}'
