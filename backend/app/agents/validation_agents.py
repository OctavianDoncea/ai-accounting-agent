...
def validate_entry(db: Session, entry: JournalEntry) -> ValidationResult:
    ...
    valid_accounts_ids = {a.id for a in db.query(ChartOfAccount).filter(ChartOfAccount.is_active.is_(True)).all()}
    ...
    for i, line in enumerate(entry.lines):
        ...
        if line.account_id not in valid_accounts_ids:
    ...
    if imbalance > BALANCE_TOLERANCE:
        errors.append(f'Entry does not balance: debits {total_debit} vs credits {total_credit} (difference {imbalance})')
    elif imbalance > 0:
        warnings.append(f'Sub-cent rounding difference of {imbalance} (within tolerance).')
